...
class BerthSerializer(TranslatedModelSerializer, munigeo_api.GeoModelSerializer):
    ...
    def get_current_reservation(self, berth):
        if self.context['request'].user.is_staff:
            return berth.berth_reservations.filter(reservation__state='confirmed').values('id', 'is_paid', 'reserver_ssn', 'reservation', 'state_updated_at', 'is_paid_at', 'key_returned', 'key_returned_at', 'reservation__reserver_name', 'reservation__begin', 'reservation__end', 'reservation__comments', 'reservation__state',).first()
        else:
            return {}
...
class BerthFilterBackend(filters.BaseFilterBackend):
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        if times.get('berth_begin', None) and times.get('berth_end', None):
            resources = BerthReservation.objects.filter(reservation__end__gte=times['berth_begin'], reservation__begin__lte=times['berth_end'], reservation__state='confirmed').values_list('reservation__resource_id', flat=True)
        elif times.get('berth_begin', None):
            resources = BerthReservation.objects.filter(reservation__end__gte=times['berth_begin'], reservation__state='confirmed').values_list('reservation__resource_id', flat=True)
        elif times.get('berth_end', None):
            resources = BerthReservation.objects.filter(reservation__begin__lte=times['berth_end'], reservation__state='confirmed').values_list('reservation__resource_id', flat=True)
...
class BerthViewSet(munigeo_api.GeoModelAPIView, viewsets.ModelViewSet):
    ...
    filter_fields = ['type']
    search_fields = ['type', 'resource__name', 'resource__name_fi', 'resource__unit__name', 'resource__unit__name_fi', 'berth_reservations__reservation__reserver_name', 'berth_reservations__reservation__reserver_email_address', 'berth_reservations__reservation__reserver_phone_number']
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        Reservation.objects.filter(~Q(state=Reservation.CANCELLED), berth_reservation__berth=berth).update(state=Reservation.CANCELLED)
